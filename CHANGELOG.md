...
* **gradle**: add `rtk gradle` command with sub-enum routing (build/test/check/dependencies) — 75-90% token savings
* **maven**: add `rtk mvn` command with Surefire XML test parser (compile/test/package/verify/install/dependency:tree) — 70-90% token savings
...
* **gradle:** add `rtk gradle` with sub-commands `build`, `test`, `check`, `dependencies` — filters to errors/failures only (75-90% savings)
* **maven:** add `rtk mvn` with sub-commands `compile`, `test`, `package`, `verify`, `install`, `dependency:tree` — Surefire XML parser with text fallback (75-90% savings)
