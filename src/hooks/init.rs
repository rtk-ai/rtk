...
use super::constants::{
    BEFORE_TOOL_KEY, CLAUDE_DIR, CLAUDE_HOOK_COMMAND, CODEX_DIR, CODEX_HOOK_COMMAND, CURSOR_HOOK_COMMAND, DROID_DIR,
...
fn run_codex_mode_with_paths(
    ...
    if !dry_run {
        ...
        println!("\n  Next: restart Codex, open `/hooks`, and trust the RTK command hook. Codex will not use it until it is trusted.");
    }
...
fn remove_codex_hook_from_file(path: &Path, ctx: InitContext) -> Result<bool> {
    ...
    if dry_run {
        println!(
            "[dry-run] would remove RTK hook from Codex hooks: {}",
            path.display()
        );
        return Ok(true);
    }
    ...
    atomic_write(path, &serialized)?;
    if verbose > 0 {
        eprintln!("Removed RTK hook from Codex hooks: {}", path.display());
    }
