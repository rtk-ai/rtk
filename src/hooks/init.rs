...
use super::constants::{
    ...
    PRE_TOOL_USE_KEY, REWRITE_HOOK_FILE, SETTINGS_JSON, SETTINGS_LOCAL_JSON, VIBE_BASH_MATCH, VIBE_DIR,
...
/// Prompt user for consent to patch a settings file
...
fn uninstall_codex_at(codex_dir: &Path, ctx: InitContext) -> Result<Vec<String>> {
    ...
    // Validate hooks.json before removing any other Codex artifacts so a
    // malformed config cannot leave a partially completed uninstall.
    let codex_hooks_root = if hooks_json_path.exists() {
        ...
        Some(
            serde_json::from_str::<serde_json::Value>(&content).with_context(|| {
                format!(
                    "Failed to parse Codex hooks.json: {}",
    ...
    if let Some(mut root) = codex_hooks_root {
        if remove_hook_command_entry(&mut root, CODEX_HOOK_COMMAND) {
            if dry_run {
                println!(
...
fn run_codex_mode_with_paths(
    ...
    if !dry_run {
        ...
        if global {
            ...
            if hooks_patched {
                println!("  hooks.json: PreToolUse hook installed for Codex CLI");
            } else {
...
fn show_claude_config() -> Result<()> {
    ...
    println!("  rtk init -g --codex         # Configure $CODEX_HOME/hooks.json + AGENTS.md + RTK.md (or ~/.codex/)");
...
mod tests {
    ...
    fn test_uninstall_codex_at_rejects_malformed_hooks_before_mutating_files() {
        ...
        assert!(err.to_string().contains("Failed to parse Codex hooks.json"));
        assert_eq!(
            fs::read_to_string(&agents_md).unwrap(),
            "# Team rules\n\n@RTK.md\n"
        );
