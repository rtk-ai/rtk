...
// Embedded Vibe hooks.toml configuration
const VIBE_HOOKS_TOML: &str = include_str!("../../hooks/vibe/hooks.toml");
...
fn vibe_hooks_toml_path(vibe_dir: &Path) -> PathBuf {
    vibe_hooks_dir(vibe_dir).join(VIBE_HOOKS_TOML_FILENAME)
}
...
fn run_vibe_mode_at(vibe_dir: &Path, ctx: InitContext) -> Result<()> {
    ...
    // Write hooks.toml configuration
    let hooks_toml_path = vibe_hooks_toml_path(vibe_dir);
    write_if_changed(&hooks_toml_path, VIBE_HOOKS_TOML, "Vibe hooks.toml", ctx)?;
