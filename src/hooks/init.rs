...
use super::constants::{
    ...
    OMP_EXTENSIONS_SUBDIR, OMP_LOCAL_DIR, OMP_PLUGIN_FILE, PI_CODING_AGENT_DIR_ENV, PI_DIR,
...
// Embedded oh-my-pi extension (auto-rewrite)
...
/// Resolve oh-my-pi config directory.
///
/// Honours `PI_CODING_AGENT_DIR` when set (the oh-my-pi runtime reads the same
/// override as the @earendil-works pi integration). Falls back to `~/.omp/agent`.
fn resolve_omp_dir() -> Result<PathBuf> {
    ...
    Ok(home.join(OMP_DIR))
}

/// Return the path to the installed oh-my-pi extension file.
fn omp_plugin_path(omp_dir: &Path) -> PathBuf {
    omp_dir.join(OMP_EXTENSIONS_SUBDIR).join(OMP_PLUGIN_FILE)
}

/// Return the oh-my-pi extension install path for the given scope.
/// global=true  → `$PI_CODING_AGENT_DIR/extensions/rtk.ts` (default `~/.omp/agent/extensions/rtk.ts`)
/// global=false → `./.omp/extensions/rtk.ts`
fn omp_plugin_path_for_scope(global: bool) -> Result<PathBuf> {
    if global {
        Ok(omp_plugin_path(&resolve_omp_dir()?))
    } else {
...
/// Write the oh-my-pi extension file if missing or outdated. Returns true if written.
fn ensure_omp_plugin_installed(path: &Path, ctx: InitContext) -> Result<bool> {
    write_if_changed(path, OMP_PLUGIN, "oh-my-pi extension", ctx)
}

/// Create the oh-my-pi extensions directory, or in dry-run mode, print a message
/// only if the directory does not yet exist.
fn ensure_omp_extensions_dir(parent: &Path, name: &str, ctx: InitContext) -> Result<()> {
    let InitContext { dry_run, .. } = ctx;
    if dry_run {
        if !parent.exists() {
            println!("[dry-run] would create {}: {}", name, parent.display());
...
/// Uninstall oh-my-pi extension for the given scope.
pub fn uninstall_omp(global: bool, ctx: InitContext) -> Result<()> {
    ...
    if plugin_path.exists() {
        if dry_run {
            println!(
                "[dry-run] would remove oh-my-pi extension: {}",
                plugin_path.display()
            );
        } else {
            // nosemgrep: filesystem-deletion -- oh-my-pi uninstall removes only the RTK-managed extension file.
            fs::remove_file(&plugin_path).with_context(|| {
                format!(
                    "Failed to remove oh-my-pi extension: {}",
                    plugin_path.display()
                )
            })?;
            if verbose > 0 {
                eprintln!("Removed oh-my-pi extension: {}", plugin_path.display());
            }
            removed.push(format!("oh-my-pi extension: {}", plugin_path.display()));
        }
    }
    ...
    if dry_run {
        ...
        println!("RTK oh-my-pi extension was not installed (nothing to remove)");
    }
    Ok(())
}

/// Install the oh-my-pi extension (hook-only; no AGENTS.md injection).
///
/// global=true  → `$PI_CODING_AGENT_DIR/extensions/rtk.ts` (default `~/.omp/agent/extensions/rtk.ts`)
/// global=false → `.omp/extensions/rtk.ts`
pub fn run_omp_mode(global: bool, ctx: InitContext) -> Result<()> {
    ...
    let plugin_path = if global {
        ...
        if let Some(parent) = path.parent() {
            ensure_omp_extensions_dir(parent, "oh-my-pi extensions directory", ctx)?;
        }
        path
    } else {
        let path = omp_plugin_path_for_scope(false)?;
        if let Some(parent) = path.parent() {
            ensure_omp_extensions_dir(parent, "local oh-my-pi extensions directory", ctx)?;
        }
        path
    };
...
fn print_omp_result(plugin_path: &Path, installed: bool) {
    ...
    println!("RTK oh-my-pi extension {}:", status);
    println!("  Extension: {}", plugin_path.display());
    println!();
    println!("omp will load the extension automatically on next start.");
    println!("Verify: omp -e {}", plugin_path.display());
}
...
mod tests {
    ...
    fn test_run_omp_mode_global_installs_plugin() {
        ...
        with_omp_dir_override(&tmp, |omp_dir| {
            ...
            assert!(plugin.exists(), "global oh-my-pi extension must be created");
    ...
    fn test_run_omp_mode_local_installs_plugin() {
        ...
        let plugin = tmp
            ...
            .join(OMP_EXTENSIONS_SUBDIR)
            .join(OMP_PLUGIN_FILE);
        assert!(plugin.exists(), "local oh-my-pi extension must be created");
    }
    ...
    fn test_run_omp_mode_global_creates_plugin_when_dir_absent() {
        ...
        let plugin = absent_dir.join(OMP_EXTENSIONS_SUBDIR).join(OMP_PLUGIN_FILE);
        assert!(
            plugin.exists(),
            "plugin must be written even when dir was absent"
        );
    ...
    fn test_run_omp_mode_global_dry_run_writes_nothing() {
        ...
        with_omp_dir_override(&tmp, |omp_dir| {
            ...
            assert!(
                ...
                "dry-run must not create the oh-my-pi extensions directory"
            );
            assert!(
                !omp_dir
                    .join(OMP_EXTENSIONS_SUBDIR)
                    .join(OMP_PLUGIN_FILE)
                    .exists(),
                "dry-run must not create the oh-my-pi extension file"
            );
        });
    }
    ...
    fn test_run_omp_mode_local_dry_run_writes_nothing() {
        ...
        assert!(
            !tmp.path().join(".omp").join(OMP_EXTENSIONS_SUBDIR).exists(),
    ...
    fn test_omp_global_uninstall_dry_run_keeps_plugin() {
        ...
        with_omp_dir_override(&tmp, |omp_dir| {
            ...
            assert!(
                plugin.exists(),
                "dry-run uninstall must not remove the oh-my-pi extension"
            );
        });
    }
    ...
    fn test_omp_local_uninstall_dry_run_keeps_plugin() {
        ...
        assert!(
            plugin.exists(),
            "dry-run uninstall must not remove the local oh-my-pi extension"
        );
    }
