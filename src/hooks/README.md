...
Owns: `rtk init` installation flows (11 agents via `AgentTarget` enum, now including Mistral Vibe and Kilo Code + 3 special modes: Gemini, Codex, OpenCode), SHA-256 integrity verification, hook version checking, audit log analysis, `rtk rewrite` CLI entry point, and TOML filter trust management.
