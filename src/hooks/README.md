...
| Kiro | `rtk init --agent kiro` (add `-g` for global) | `steering/rtk.md` + `hooks/rtk-rewrite.json` under `~/.kiro/` with `-g`, else `.kiro/` (global hook read by Kiro CLI only) | -- |
...
- `hook_cmd.rs` — maps decisions to each agent's JSON protocol, including Codex `updatedInput`; `run_kiro()` handles the Kiro PreToolUse hook protocol and returns the exit code (0 or `KIRO_BLOCK_EXIT`)
