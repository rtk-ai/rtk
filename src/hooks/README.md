...
| Codex | `rtk init --codex` | `RTK.md` + `hooks.json` in `$CODEX_HOME` or `~/.codex` | `AGENTS.md`, existing `hooks.json` |
...
| Codex (`rtk hook codex`) | Native Codex policy applies after rewrite | allow with `updatedInput` |
...
- `hook_cmd.rs` — maps rewrites and verdicts to each agent's JSON hook protocol (Codex/Copilot/Gemini)
