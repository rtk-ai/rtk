...
Hook warnings are rate-limited with the default marker in RTK's local data directory. Hermetic test runners may set `RTK_HOOK_WARN_MARKER` to a non-empty marker-file path. An unset or empty value preserves the default location and warning behavior.

`RTK_HOOK_WARN_HOME` optionally sets the non-Claude integration home inspected by that warning check. It has the same non-empty override and unset/empty fallback semantics; it does not change hook installation paths.

## Adding New Functionality
To add support for a new AI coding agent: (1) add the hook installation logic to `init.rs` following the existing agent patterns, (2) if the agent requires a custom hook protocol (like Gemini's `BeforeTool` or Vibe's `pre_tool`), add a processor function in `hook_cmd.rs` and a matching `HookCommands::<Agent>` variant + `AgentTarget::<Agent>` enum entry in `main.rs`, (3) if the agent has installable permission surfaces (denylist / allowlist), wire them into `permissions.rs::check_command_for` via a new `Host::<Agent>` variant, and (4) update `integrity.rs` with the expected hash for the new hook file. Note that `hook_check.rs::maybe_warn()` evaluates Claude hook status: recognized non-Claude integrations suppress only the missing-hook warning, never an outdated-Claude warning. Test by running `rtk init` in a fresh environment and verifying the hook rewrites commands correctly in the target agent.
