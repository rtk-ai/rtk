...
fn is_rtk_hook_command(command: &str, agent: &str) -> bool {
    ...
    // `shell_split` intentionally treats backslashes as shell escapes, which
    // would erase separators in a quoted Windows path. Read the raw first
    // token for basename detection while still using `shell_split` to validate
    // the hook arguments.
