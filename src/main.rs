...
enum Commands {
    ...
    Read {
        ...
        #[arg(short = 'n', long)]
        line_numbers: bool,
        /// Show all non-printing characters (passthrough to native cat)
        #[arg(short = 'A', long, conflicts_with_all = ["level", "max_lines", "tail_lines"])]
        show_all: bool,
    },
...
fn run_cli() -> Result<i32> {
    ...
    let code = match cli.command {
        ...
        Commands::Read {
            ...
            if show_all {
                // `-vet` is the portable equivalent of GNU cat's `-A`.
                let mut args = vec![OsString::from("-vet")];
                if line_numbers {
                    args.push(OsString::from("-n"));
                }
                args.extend(files.iter().map(|file| file.as_os_str().to_owned()));
                return core::runner::run_passthrough("cat", &args, cli.verbose);
            }
...
mod tests {
    ...
    #[test]
    fn test_try_parse_read_show_all_flag() {
        let cli = Cli::try_parse_from(["rtk", "read", "-A", "example.py"]).unwrap();
        match cli.command {
            Commands::Read {
                files, show_all, ..
            } => {
                assert!(show_all);
                assert_eq!(files, vec![PathBuf::from("example.py")]);
            }
