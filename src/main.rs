...
mod tests {
    ...
    #[test]
    fn test_lua_commands_forward_native_help_flag() {
        let cli = Cli::try_parse_from(["rtk", "luacheck", "--help"]).unwrap();
        match cli.command {
            Commands::Luacheck { args } => assert_eq!(args, vec!["--help"]),
            _ => panic!("Expected Commands::Luacheck"),
        }

        let cli = Cli::try_parse_from(["rtk", "busted", "--help"]).unwrap();
        match cli.command {
            Commands::Busted { args } => assert_eq!(args, vec!["--help"]),
            _ => panic!("Expected Commands::Busted"),
        }
    }
}
