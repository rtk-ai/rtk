...
/// The two files allowed to name `dirs`' data directory: the resolver itself,
/// and this scanner, which carries the marker as a literal.
const RESOLVERS: [&str; 2] = ["src/core/constants.rs", "src/core/test_isolation/mod.rs"];

/// Resolving the data directory from `dirs` bypasses `constants::data_dir`'s
/// test redirect, so a test reaching such a path writes to — or, in
/// `telemetry forget`'s case, deletes — the developer's own.
#[test]
fn only_constants_resolves_the_data_directory() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let mut offenders = Vec::new();
    ...
    for path in rust_files(&root.join("src")) {
        ...
        let src = std::fs::read_to_string(&path).expect("read source");
        if src.contains("data_local_dir()") {
            offenders.push(relative);
        }
    }
    ...
    assert!(
        offenders.is_empty(),
        "these files resolve rtk's data directory themselves instead of \
         through `constants::data_dir()`, so a test reaching them lands in the \
