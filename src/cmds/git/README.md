...
- The compact `git status` listing reads `limits.status_max_files` (tracked, default 15) and `limits.status_max_untracked` (default 10) from `core/config`. What a cap leaves off is counted (`... +N more`) and recoverable through the recall hint; conflicted paths are never counted against either cap
