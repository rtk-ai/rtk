...
- Paths whose diff output RTK **parses** build their command with `git_diff_cmd()`, which appends `DIFF_HARDENING` (`--no-ext-diff --no-textconv --src-prefix=a/ --dst-prefix=b/`) after the subcommand. Without it, a user's `diff.external` (difftastic, delta), a `textconv` driver or `diff.noprefix` makes `compact_diff()` match nothing and silently emit an empty diff. These are subcommand flags, not git globals — `git --no-ext-diff status` is an error — so they must never move into `git_cmd()`
- **Passthrough** paths (`git diff --stat` / `--no-compact`, `git log -p`, and any unmodelled subcommand via `GitCommands::Other`) deliberately skip the hardening: they print git's bytes verbatim, so they must keep honouring the user's diff configuration. `tests/guard_integration_test.rs` byte-compares `rtk git log -p` against raw `git log -p`
