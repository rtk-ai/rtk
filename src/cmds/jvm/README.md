...
### Surefire XML recovery (`surefire_recovery.rs`, `surefire_reports.rs`)

Maven's stdout is not always a complete record of what failed: a forked-VM crash, parallel forks interleaving output, `-q`, `<redirectTestOutputToFile>` or `-Dmaven.test.failure.ignore=true` can hide per-test detail that `target/surefire-reports/TEST-*.xml` still carries. After the stdout filter runs (Test / Package / quiet routes), `surefire_recovery::recover_missing_failures` parses the fresh `surefire-reports` / `failsafe-reports` XML (root + depth-1 reactor modules; mtime-gated to the run's start so stale reports never leak) and appends only the failures whose `Class.method` the filtered text does not mention, in `[ERROR]`-prefixed native style: exception header plus the stack trace verbatim, head-capped to `MAX_STACK_TRACE_LINES`, at most `CAP_WARNINGS` blocks (`… +N more recovered failures` tail). When stdout carried no `Tests run:` aggregate at all, one is restored from the XML. The block is appended only while the result still fits under the raw size (the runner's `never_worse` ceiling); otherwise the filtered text is left as is. When stdout already covers every failure — the common case — the output is unchanged byte for byte.
