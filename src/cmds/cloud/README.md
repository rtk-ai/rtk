...
- `container.rs` handles Docker, Podman, Kubernetes, and OpenShift; `DockerCommands`, `PodmanCommands`, `KubectlCommands`, and `OcCommands` sub-enums in `main.rs` route to `container::run()` -- Podman reuses the same filter functions as Docker (identical CLI output format); passthrough for unknown subcommands
