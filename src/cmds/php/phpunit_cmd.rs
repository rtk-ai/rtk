...
fn phpunit_run_options() -> runner::RunOptions<'static> {
...
mod tests {
    ...
    #[test]
    fn phpunit_failure_options_preserve_raw_diagnostics() {
        let options = phpunit_run_options();

        assert!(options.filter_stdout_only);
        assert!(options.skip_filter_on_failure);
        assert_eq!(options.tee_label, Some("phpunit"));
    }
