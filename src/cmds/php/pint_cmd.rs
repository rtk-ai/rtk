...
#[derive(Deserialize)]
struct PintFile {
    // Pint 鈮?~1.14 renamed the JSON keys: name鈫抪ath, appliedFixers鈫抐ixers.
...
fn pint_run_options() -> runner::RunOptions<'static> {
...
mod tests {
    ...
    #[test]
    fn pint_failure_options_preserve_raw_diagnostics() {
        let options = pint_run_options();

        assert!(options.filter_stdout_only);
        assert!(options.skip_filter_on_failure);
        assert_eq!(options.tee_label, Some("pint"));
    }
    ...
    fn test_pint_current_schema_path_fixers() {
        // Pint 鈮?~1.14 emits path/fixers instead of name/appliedFixers.
