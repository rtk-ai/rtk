...
pub fn filter_test_output(output: &str, runner: PhpTestRunner) -> String {
    match runner {
        PhpTestRunner::Pest | PhpTestRunner::Phpunit => filter_test_runner_output(output),
        PhpTestRunner::Unknown => filter_output(output),
    }
}
...
mod tests {
    ...
    fn test_artisan_cleanup() {
        ...
        let filtered = filter_output(out);
    ...
    fn test_artisan_test_prefers_runner_filter() {
        ...
        let filtered = filter_test_output(output, PhpTestRunner::Phpunit);
