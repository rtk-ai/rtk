...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    ...
    runner::run_filtered(
        ...
        paratest_run_options(),
    )
}

fn paratest_run_options() -> runner::RunOptions<'static> {
...
mod tests {
    ...
    #[test]
    fn paratest_failure_options_preserve_raw_diagnostics() {
        let options = paratest_run_options();

        assert!(options.filter_stdout_only);
        assert!(options.skip_filter_on_failure);
        assert_eq!(options.tee_label, Some("paratest"));
    }
