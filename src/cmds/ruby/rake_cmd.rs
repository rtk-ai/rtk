...
/// A Rails project is detected by its conventional binstub. An app without one
/// stays on `rake`, which is always safe; `Gemfile.lock` would catch it if that
/// ever matters.
...
6 runs, 5 assertions, 2 failures, 1 errors, 0 skips"#;
    ...
    fn test_select_runner_multiple_with_line_numbers() {
        let (tool, _) = select_runner(
            &args("test test/models/post_test.rb:15 test/models/user_test.rb:30"),
            true,
        );
        assert_eq!(tool, "rails");
    }
    ...
    fn test_nonzero_exit_without_parsed_failures_passes_through() {
        ...
        assert!(
            ...
            "raw output expected: {result}"
        );
    }

    #[test]
    fn test_crash_during_run_passes_through() {
        let output = "Run options: --seed 123\n\n\
            # Running:\n\n\
            ..\n\
            Killed";
        let result = filter_minitest_output(output, 137);
