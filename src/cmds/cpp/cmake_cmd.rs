...
static CMAKE_PROBE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^-- (Check for working|Detecting|Looking for|Found|Performing Test|Checking)")
        .unwrap()
});
...
fn filter_build(raw: &str, args: &[String], exit_code: i32) -> String {
    ...
    for line in raw.lines() {
        ...
        if line.contains("error:")
            ...
            || line
                .to_ascii_lowercase()
                .contains("build stopped: subcommand failed")
        {
            out.push(line.to_string());
            emitted_diag = true;
            diag_context = 2;
...
fn filter_configure(raw: &str, exit_code: i32) -> String {
    ...
    for line in raw.lines() {
        ...
        if trimmed.starts_with("CMake Error") || trimmed.starts_with("CMake Warning") {
            ...
            error_context = if trimmed.starts_with("CMake Error") {
                6
            } else {
                0
            };
...
mod tests {
    ...
    fn test_savings_build_success() {
        let raw = (0..50)
            .map(|i| {
                format!(
                    "[{:>3}%] Building CXX object CMakeFiles/lib.dir/file{}.cpp.o",
                    i * 2,
                    i
                )
            })
    ...
    fn msvc_and_link_diagnostics_survive() {
        let raw =
            "C:\\src\\main.cpp(42,7): error C2065: name\nLINK : fatal error LNK1104: missing.lib\n";
