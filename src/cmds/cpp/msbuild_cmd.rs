...
static MSVC_COMPILER_RE: LazyLock<Regex> = LazyLock::new(|| {
    // Compiler: file(line): error|warning C1234: message [project.vcxproj]
    Regex::new(r"^(.+)\((\d+)(?:,\d+)?\): (error|warning|fatal error) (C\d+): (.+?)(?:\s+\[.+\])?$").unwrap()
});
    // Linker: module : error|fatal error LNK1234: message
static MSVC_LINKER_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^(.+) : (error|fatal error) (LNK\d+): (.+)$").unwrap());
    // Linker tool (no file prefix): "LINK : fatal error LNK1104: ..."
static MSVC_LINK_TOOL_RE: LazyLock<Regex> = LazyLock::new(|| {
...
static RC_DIAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^(.+)\((\d+)(?:,\d+)?\): (warning|error|fatal error) (RC\d+): (.+?)(?:\s+\[.+\])?$")
        .unwrap()
});
    // MSBuild-style diagnostics: path.vcxproj(123,5): error MSB3073: ...
static MSBUILD_DIAG_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(
        r"^(.+?)\((\d+)(?:,(\d+))?\): (warning|error|fatal error) ((?:MSB|PRJ|CVT|LNK|RC|C)\d+): (.+)$"
    ).unwrap());
static MSB3073_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\b(MSB3073|MSB3721)\b").unwrap());
static EXIT_CODE_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\bexited with code\s+(\d+)\b").unwrap());
static PROJECT_ON_NODE_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"^Project \"(.+?)\" on node \d+ \((.+?) target\(s\)\)\."#).unwrap()
});
static DONE_BUILDING_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r#"^Done Building Project \"(.+?)\" \(.+\) -- (FAILED|SUCCESSFUL)\."#).unwrap()
});
    // Build FAILED. or Build succeeded.
static BUILD_RESULT_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"^Build (FAILED|succeeded)\.").unwrap());
    // "    N Error(s)" or "    N Warning(s)"
static ERR_WARN_COUNT_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^\s+\d+\s+(Error|Warning)\(s\)").unwrap());
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    let cmd = msbuild_command(args);
    if verbose > 0 {
        eprintln!("Running: msbuild {}", args.join(" "));
    }
    ...
    runner::run_filtered_with_exit(
        ...
        "msbuild",
        &args.join(" "),
        move |raw, exit_code| filter_output(raw, &args_owned, exit_code),
        RunOptions::with_tee("msbuild").preserve_filtered_failure_output(),
    )
}
...
pub(crate) fn filter_output(raw: &str, args: &[String], exit_code: i32) -> String {
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        if PROJECT_ON_NODE_RE.is_match(trimmed) {
            continue;
        }
        ...
        if let Some(diag) = parse_diag_line(trimmed, idx) {
            diags.push(diag);
            continue;
        }
    ...
    if exit_code != 0 && !has_errors {
        return failure_fallback("msbuild", exit_code, raw);
    }
    ...
    if !has_errors && build_result.is_none() && diags.is_empty() {
        ...
        return format!(
            "msbuild: no output captured \u{2014} rerun with a linker-enabled target\n\
             [target: {}]",
            target
        );
    }
...
fn parse_diag_line(line: &str, idx: usize) -> Option<MsbuildDiag> {
    ...
    let project = project_from_suffix;
...
fn dedup_diags(diags: &[MsbuildDiag]) -> Vec<MsbuildDiag> {
    ...
    for d in diags {
        let key = format!("{}|{}|{}", d.project.as_deref().unwrap_or(""), d.code, d.raw);
        if !seen.insert(key) {
            continue;
        }
        out.push(d.clone());
...
mod tests {
    ...
    #[test]
    fn msbuild_preserves_original_arguments() {
        let args = vec!["app.sln".to_string(), "/t:Link".to_string(), "/p:Configuration=Release".to_string()];
        let command = msbuild_command(&args);
        let actual: Vec<String> = command
            .get_args()
            .map(|arg| arg.to_string_lossy().into_owned())
            .collect();
        assert_eq!(actual, args);
        assert!(actual.contains(&"/t:Link".to_string()));
        assert!(!actual.contains(&"/t:Build".to_string()));
    }
    ...
    fn test_compiler_error_strips_project_suffix() {
        ...
        let out = filter_output(raw, &args, 1);
    ...
    fn test_success_compact() {
        ...
        let out = filter_output(raw, &args, 0);
    ...
    #[test]
    fn identical_diagnostics_from_distinct_projects_are_not_deduped() {
        let raw = "C:\\src\\main.cpp(42): error C2065: name [C:\\a\\a.vcxproj]\nC:\\src\\main.cpp(42): error C2065: name [C:\\b\\b.vcxproj]\n";
        let out = filter_output(raw, &["all.sln".into()], 1);
        assert_eq!(out.matches("C:\\src\\main.cpp(42): error C2065").count(), 3);
    }

    #[test]
    fn projectless_linker_error_has_no_stale_project() {
        let raw = "Project \"a.vcxproj\" on node 1 (Build target(s)).\nLINK : fatal error LNK1104: missing.lib\n";
        let out = filter_output(raw, &["all.sln".into()], 1);
        assert!(out.contains("LNK1104"));
        assert!(!out.contains("project: a.vcxproj"));
    }

    #[test]
    fn unknown_and_empty_nonzero_msbuild_output_is_failure() {
        assert!(filter_output("localized failure", &[], 5).contains("msbuild: failed (exit 5)"));
        assert_eq!(filter_output("", &[], 1), "msbuild: failed (exit 1)");
    }
}
