...
static MSVC_COMPILER_RE: LazyLock<Regex> = LazyLock::new(|| {
    ...
    Regex::new(r"^(.+)\((\d+)(?:,\d+)?\): (error|warning|fatal error) (C\d+): (.+?)(?:\s+\[.+\])?$")
        .unwrap()
});
...
static RC_DIAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"^(.+)\((\d+)(?:,\d+)?\): (warning|error|fatal error) (RC\d+): (.+?)(?:\s+\[.+\])?$",
    )
    .unwrap()
});
// MSBuild-style diagnostics: path.vcxproj(123,5): error MSB3073: ...
static MSBUILD_DIAG_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"^(.+?)\((\d+)(?:,(\d+))?\): (warning|error|fatal error) ((?:MSB|PRJ|CVT|LNK|RC|C)\d+): (.+)$"
    ).unwrap()
});
static MSB3073_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)\b(MSB3073|MSB3721)\b").unwrap());
static EXIT_CODE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)\bexited with code\s+(\d+)\b").unwrap());
...
static BUILD_RESULT_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^Build (FAILED|succeeded)\.").unwrap());
