...
/// Detect whether to use ./mvnw or mvn.
/// Prefers ./mvnw if it exists in the current directory.
...
pub fn run_compile(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = build_mvn_command("compile", args);

    if verbose > 0 {
        eprintln!("Running: mvn compile {}", args.join(" "));
    }

    let output = cmd
        .output()
        .context("Failed to run mvn compile. Is Maven installed?")?;
    ...
    let filtered = filter_mvn_compile(&raw);

    if let Some(hint) = crate::core::tee::tee_and_hint(&raw, "mvn_compile", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
    ...
    timer.track(
        &format!("mvn compile {}", args.join(" ")),
        &format!("rtk mvn compile {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
pub fn run_test(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = build_mvn_command("test", args);
...
pub fn run_package(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = build_mvn_command("package", args);

    if verbose > 0 {
        eprintln!("Running: mvn package {}", args.join(" "));
    }

    let output = cmd
        .output()
        .context("Failed to run mvn package. Is Maven installed?")?;
    ...
    let exit_code = output
        ...
        .code()
        .unwrap_or(if output.status.success() { 0 } else { 1 });
    let filtered = filter_mvn_package(&raw);

    if let Some(hint) = crate::core::tee::tee_and_hint(&raw, "mvn_package", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
    ...
    timer.track(
        &format!("mvn package {}", args.join(" ")),
        &format!("rtk mvn package {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
pub fn run_clean(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = build_mvn_command("clean", args);

    if verbose > 0 {
        eprintln!("Running: mvn clean {}", args.join(" "));
    }

    let output = cmd
        .output()
        .context("Failed to run mvn clean. Is Maven installed?")?;
    ...
    let exit_code = output
        ...
        .code()
        .unwrap_or(if output.status.success() { 0 } else { 1 });
    let filtered = filter_mvn_clean(&raw);

    if let Some(hint) = crate::core::tee::tee_and_hint(&raw, "mvn_clean", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
    ...
    timer.track(
        &format!("mvn clean {}", args.join(" ")),
        &format!("rtk mvn clean {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
pub fn run_integration_test(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = build_mvn_command("integration-test", args);

    if verbose > 0 {
        eprintln!("Running: mvn integration-test {}", args.join(" "));
    }

    let output = cmd
        .output()
        .context("Failed to run mvn integration-test. Is Maven installed?")?;
    ...
    let exit_code = output
        ...
        .code()
        .unwrap_or(if output.status.success() { 0 } else { 1 });
    let filtered = filter_mvn_test(&raw); // Same filter as test (Surefire/Failsafe format)

    if let Some(hint) = crate::core::tee::tee_and_hint(&raw, "mvn_integration_test", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
    ...
    timer.track(
        &format!("mvn integration-test {}", args.join(" ")),
        &format!("rtk mvn integration-test {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
pub fn run_install(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = build_mvn_command("install", args);

    if verbose > 0 {
        eprintln!("Running: mvn install {}", args.join(" "));
    }

    let output = cmd
        .output()
        .context("Failed to run mvn install. Is Maven installed?")?;
    ...
    let exit_code = output
        ...
        .code()
        .unwrap_or(if output.status.success() { 0 } else { 1 });
    let filtered = filter_mvn_install(&raw);

    if let Some(hint) = crate::core::tee::tee_and_hint(&raw, "mvn_install", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
    ...
    timer.track(
        &format!("mvn install {}", args.join(" ")),
        &format!("rtk mvn install {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
pub fn run_dependency_tree(args: &[String], verbose: u8) -> Result<i32> {
    ...
    let mut cmd = build_mvn_command("dependency:tree", args);
    ...
    let output = cmd
        .output()
        .context("Failed to run mvn dependency:tree. Is Maven installed?")?;
    ...
    let exit_code = output
        ...
        .code()
        .unwrap_or(if output.status.success() { 0 } else { 1 });
    let filtered = filter_mvn_dependency_tree(&raw);

    if let Some(hint) = crate::core::tee::tee_and_hint(&raw, "mvn_dependency_tree", exit_code) {
        println!("{}\n{}", filtered, hint);
    } else {
        println!("{}", filtered);
    }
    ...
    timer.track(
        &format!("mvn dependency:tree {}", args.join(" ")),
        &format!("rtk mvn dependency:tree {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
fn is_maven_noise(line: &str) -> bool {
    ...
    if content.starts_with("Reactor Build Order:")
        || content.starts_with("Reactor Summary")
        || content == "Building jar:"
    {
        return true;
    }
    ...
    if content.starts_with("Building ") || content.starts_with("Finished at:") {
        return true;
    }
...
/// Returns the LAST (aggregate) "Tests run:" line as (tests_run, failures, errors, skipped).
/// Maven outputs per-class results first, then a final summary — we want the final one.
fn parse_test_summary(output: &str) -> Option<(usize, usize, usize, usize)> {
    let mut result = None;

    for line in output.lines() {
        let trimmed = line.trim();
        // Strip [INFO] or [ERROR] prefix
        let content = trimmed
            .strip_prefix("[INFO] ")
            .or_else(|| trimmed.strip_prefix("[ERROR] "))
            .unwrap_or(trimmed);

        // Match: "Tests run: 42, Failures: 0, Errors: 0, Skipped: 2"
        if content.starts_with("Tests run:") {
            ...
            result = Some((tests, failures, errors, skipped));
        }
    }
...
fn extract_test_failures(output: &str) -> Vec<(String, Vec<String>)> {
    ...
    for line in output.lines() {
        ...
        if trimmed.starts_with("[ERROR] ") && !trimmed.contains("Tests run:") {
            let content = &trimmed[8..]; // Strip "[ERROR] "

            // If we have a pending test, flush it
            if let Some(test) = current_test.take() {
                failures.push((test, current_output.clone()));
                current_output.clear();
            }

            // Check if this looks like a test name (contains parens or common patterns)
            if content.contains('(') || content.contains('.') {
                current_test = Some(content.to_string());
                in_failure_section = true;
            } else {
                // Just an error line, collect it
...
fn compact_dependency_line(line: &str) -> String {
    ...
    if parts.len() >= 4 {
        ...
        // Compact to: group:artifact:version
        let group = parts[0];
        let name = parts[1];
