...
pub fn run(
    ...
    {
        if let Some(res) = try_grep_yaml(&patterns, &paths, &extra_args, max_results, max_line_len)
        {
            print!("{}", res.output);
            timer.track(
...
fn yaml_mode_supported(extra_args: &[String]) -> bool {
    extra_args.iter().all(|a| a == "-i" || a == "--ignore-case")
}
...
fn try_grep_yaml(
    ...
    let case_insensitive = extra_args.iter().any(|a| a == "-i" || a == "--ignore-case");
...
mod tests {
    ...
    fn test_grep_yaml_case_insensitive() {
        ...
        assert_eq!(res.exit_code, 0);
        assert!(
            res.output.contains("Service.Enabled: true"),
            "got: {}",
            res.output
        );
    }
