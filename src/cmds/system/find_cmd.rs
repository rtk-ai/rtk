...
mod tests {
    ...
    fn find_works_after_git_init() {
        ...
        // `rtk find ./ -name "message.csv"` used to return nothing once the
        // directory became a git repo and an ignore rule (*.csv in the
        // reporter's global gitignore) swallowed the match. The match must be
        ...
        std::fs::write(root.join("message.csv"), "test").unwrap();
        let root_s = root.to_string_lossy().into_owned();

        let (files, filtered) = native_walk(&root_s, "message.csv", None, false, false, true);
        assert!(
            files.is_empty(),
            "gitignored match stays out of the main listing: {files:?}"
        );
        assert_eq!(
            filtered,
            vec!["message.csv".to_string()],
            "#2589: the match must be disclosed, not silently dropped"
        );
    }
