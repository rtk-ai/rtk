...
static DIAGNOSTIC_LINE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r": (?:warning|error|note):").expect("diagnostic regex"));

// "In file included from" context lines preceding a diagnostic
static INCLUDE_CHAIN: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^(?:In file included from|                 from)\s").expect("include chain regex")
});

// make[N]: lines to strip
static MAKE_BRACKET: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^make\[\d+\]:").expect("make bracket regex"));

// Lines to drop unconditionally (blank-line runs handled separately)
static NOTHING_TO_DO: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"^(?:Nothing to be done|make: Nothing)").expect("nothing to do regex")
});

// Link / install / phase boundaries — flush compile group, keep verbatim
static LINK_OR_INSTALL: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?x)
    ^(?:
        /usr/bin/install\b |
...
pub fn filter_make_output(raw: &str) -> String {
    ...
    let flush_group =
        |output: &mut Vec<String>, count: &mut usize, warns: &mut usize, errs: &mut usize| {
            if *count == 0 {
                return;
            }
...
/// Head and tail budgets for the no-compiler fallback. `make test` and
/// `make check` emit thousands of progress lines and put the verdict last, so a
/// head-only cap drops exactly the part worth keeping.
const FALLBACK_HEAD_LINES: usize = 10;
...
fn strip_chrome(raw: &str) -> String {
    ...
    if lines.len() <= FALLBACK_HEAD_LINES + FALLBACK_TAIL_LINES {
        return lines.join("\n");
    }

    let omitted = lines.len() - FALLBACK_HEAD_LINES - FALLBACK_TAIL_LINES;
    let mut out: Vec<&str> = Vec::with_capacity(FALLBACK_HEAD_LINES + FALLBACK_TAIL_LINES + 1);
    out.extend_from_slice(&lines[..FALLBACK_HEAD_LINES]);
    let marker = format!("... ({} lines omitted)", omitted);
    out.push(&marker);
    out.extend_from_slice(&lines[lines.len() - FALLBACK_TAIL_LINES..]);
    out.join("\n")
}
...
mod tests {
    ...
    #[test]
    fn test_fallback_keeps_tail_summary() {
        // `make test` on a PHPT-style suite: thousands of progress lines, no
        // compiler invocations, verdict last. The fallback must not return the
        // whole stream, and must not drop the verdict.
        let mut raw = String::from("Running tests\n");
        for i in 0..3000 {
            raw.push_str(&format!("TEST {}/3000 [ext/foo/tests/bar_{}.phpt]\n", i, i));
        }
        raw.push_str("=====================\nFAILED TEST SUMMARY\nbar_42.phpt\nTests failed: 1\n");

        let output = filter_make_output(&raw);

        assert!(
            output.lines().count() <= FALLBACK_HEAD_LINES + FALLBACK_TAIL_LINES + 1,
            "fallback must cap output, got {} lines",
            output.lines().count()
        );
        assert!(output.starts_with("Running tests"), "head missing");
        assert!(output.contains("lines omitted"), "omission marker missing");
        assert!(output.ends_with("Tests failed: 1"), "verdict missing");
        assert!(output.contains("FAILED TEST SUMMARY"));

        let savings = 100.0 - (count_tokens(&output) as f64 / count_tokens(&raw) as f64 * 100.0);
        assert!(
            savings >= 60.0,
    ...
    #[test]
    fn test_fallback_short_output_is_untouched() {
        let raw = "make: Entering\nbuilding docs\nmake: done\n";
        let output = filter_make_output(raw);
        assert_eq!(output, "make: Entering\nbuilding docs\nmake: done");
    }
    ...
    fn test_preserves_warnings_verbatim() {
        ...
        let output = filter_make_output(input);
        assert!(
            output.contains("warning: unused variable 'x'"),
            "Warning line must appear verbatim"
        );
        assert!(
            output.contains("compiled"),
            "Should still have compile summary"
        );
    }
    ...
    fn test_preserves_errors_verbatim() {
        ...
        assert!(
            ...
            "Error line must appear verbatim"
        );
    }
...
";
        let output = filter_make_output(input);
        assert!(
            output.contains("In file included from"),
            "include chain should be preserved"
        );
...
";
        let output = filter_make_output(input);
        assert!(
            output.contains("/usr/bin/install"),
            "Install line should be verbatim"
        );
        assert!(
            output.contains("compiled 3 files"),
            "Compile group before install"
        );
    }
...
";
        ...
        assert!(
            output.contains("compiled 5 files") || output.contains("compiled"),
            "Should group ar/ranlib with gcc"
        );
    }
...
";
        ...
        assert!(
            output.contains("compiled"),
            "Should summarize even below threshold"
        );
    }
}
