...
- `search.rs` backs both `rtk grep` and `rtk rg`: it runs the invoked engine (never substituting one for the other) and groups its output, reading `core/config` for `limits.grep_max_results` and `limits.grep_max_per_file`. `rtk grep` additionally supports opt-in `--agent-safe` (80 total matches, 5/file, 240 Unicode characters/line), `--files-only`, `--count-by-file`, `--top-files N`, explicit limits, `--all`, `--full-lines`, and deterministic `--json` output using `rtk.grep.v1`. Structured capture is bounded to 10 MiB per stream. Format-altering flags (`-c`, `-l`, `-L`, `-o`, `-Z`) bypass RTK filtering and run raw; `rtk rg --json` remains native ripgrep JSON.
