...
use crate::core::utils::{fallback_tail, resolved_command, strip_ansi, truncate};
use anyhow::Result;
...
pub fn run(args: &[String], verbose: u8) -> Result<i32> {
    ...
    runner::run_filtered(
        ...
        runner::RunOptions::with_tee("busted"),
    )
}
...
pub(crate) fn filter_busted_output(output: &str) -> String {
    ...
    if failures.is_empty() {
        if summary.is_none() {
            return fallback_tail(&clean, "busted", 8);
        }
...
3 successes / 1 failure / 1 error / 0 pending : 0.020 seconds"#;
    ...
    #[test]
    fn test_filter_busted_stderr_only_error_falls_back_to_raw_tail() {
        let output = "Access is denied.\n";
        let result = filter_busted_output(output);

        assert!(result.contains("Access is denied."));
        assert!(!result.contains("0 failures/errors"));
    }
