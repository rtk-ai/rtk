...
fn filter_next_build(output: &str) -> String {
    static ROUTE_PATTERN: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"^[\s┌├└│]*([○●◐λƒ])\s+/").unwrap());
    ...
    for line in clean_output.lines() {
        // Count route types by symbol, including box-drawing prefixes used by Next 15.
...
"#;
    ...
    #[test]
    fn test_filter_next_build_counts_prefixed_dynamic_routes() {
        let output = r#"
Route (app)                    Size     First Load JS
┌ ƒ /                          184 B         105 kB
...
"#;
        ...
        let result = filter_next_build(output);
        assert!(result.contains("3 routes (1 static, 2 dynamic)"));
    }
}
