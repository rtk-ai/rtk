...
/// Diagnostic header: `path/to/file.ts:10:5 lint/suspicious/noDoubleEquals`
/// (or the same without the line:col).
static BIOME_DIAG: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?m)^[^\s].*?\s+(lint/\S+|a11y/\S+|nursery/\S+|style/\S+|suspicious/\S+|correctness/\S+|complexity/\S+|performance/\S+|security/\S+)",
    )
    .expect("biome diagnostic regex")
});
/// Biome's own tally: `Found 4 errors.` / `Found 2 warnings.`
static BIOME_SUMMARY: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?i)Found\s+(\d+)\s+(error|warning|diagnostic)").expect("biome summary regex")
});
/// File path opening a diagnostic block.
static BIOME_FILE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?m)^(\S+\.\w{1,6})(?::\d+(?::\d+)?)?").expect("biome file regex")
});
...
fn filter_biome_output(output: &str) -> String {
    // Extract rule names
    let mut by_rule: HashMap<String, usize> = HashMap::new();
    for cap in BIOME_DIAG.captures_iter(output) {
    ...
    for cap in BIOME_FILE.captures_iter(output) {
    ...
    for cap in BIOME_SUMMARY.captures_iter(output) {
...
Found 4 errors."#;
        ...
        assert!(
            ...
            "should have Biome header: {}",
            result
        );
        assert!(
            result.contains("4 issues"),
            "should report 4 issues: {}",
            result
        );
        assert!(
