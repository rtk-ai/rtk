...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx)\s+)?(eslint|lint)(\s|$)",
        rtk_cmd: "rtk lint",
        rewrite_prefixes: &[
            "eslint",
            "lint",
    ...
    // Biome must NOT route to `rtk lint` (the ESLint adapter): the rewrite strips
    // the "biome" token, so `rtk lint` falls back to ESLint and dies in Biome-only
    // projects (#1489). Direct invocations route to `rtk biome` (TOML filter);
    // script-runner forms like `npm run biome` are left to the npm/pnpm rules above.
    RtkRule {
        pattern: r"^((npx|pnpx|npm\s+(exec|x)|pnpm\s+(exec|dlx))\s+)?biome(\s|$)",
        rtk_cmd: "rtk biome",
        rewrite_prefixes: &[
            "biome",
            "npm exec biome",
