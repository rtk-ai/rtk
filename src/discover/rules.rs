...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^bazel\s+(?:(?:--[^\s=]+(?:=\S+|\s+\S+)?|-+\S+)\s+)*(build|test|lint|run|query)(?:\s|$)",
        rtk_cmd: "rtk bazel",
        ...
        subcmd_status: &[("query", RtkStatus::Passthrough)],
    },
    RtkRule {
        pattern: r"^bazelisk\s+(?:(?:--[^\s=]+(?:=\S+|\s+\S+)?|-+\S+)\s+)*(build|test|lint|run|query)(?:\s|$)",
        rtk_cmd: "rtk bazelisk",
        rewrite_prefixes: &["bazelisk"],
        category: "Build",
        savings_pct: 62.0,
        subcmd_savings: &[
            ("test", 75.0),
