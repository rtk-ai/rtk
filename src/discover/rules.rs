...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^pnpm\s+(exec|i|install|list|ls|outdated)(\s|$)",
        rtk_cmd: "rtk pnpm",
    ...
    RtkRule {
        pattern: r"^pnpm\s+(run|run-script)\s+typecheck(\s|$)",
        rtk_cmd: "rtk pnpm typecheck",
        rewrite_prefixes: &["pnpm run-script typecheck", "pnpm run typecheck"],
        category: "Build",
        savings_pct: 83.0,
        ..RtkRule::DEFAULT
    },
