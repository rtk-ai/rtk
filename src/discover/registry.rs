...
// invocations fail to match so the segment is passed through to native `tail`,
// which already handles `==> name <==` banners that `rtk read` cannot reproduce.
...
fn rewrite_line_range(cmd: &str) -> Option<String> {
    // Preserve native head's exact-first-N semantics. `rtk read --max-lines`
    // uses smart truncation and may elide interior lines.
