...
mod tests {
    ...
    fn test_rewrite_pnpm_command() {
        let commands = vec!["exec", "i", "install", "list", "ls", "outdated"];
        for command in commands {
            assert_eq!(
    ...
    #[test]
    fn test_rewrite_unknown_pnpm_scripts_stay_raw() {
        for command in ["pnpm run custom", "pnpm run-script custom"] {
            assert_eq!(
                rewrite_command_no_prefixes(command, &[]),
                None,
                "Unexpected rewrite for command: {command}"
            );
        }
    }

    #[test]
    fn test_rewrite_pnpm_run_typecheck() {
        for command in ["pnpm run typecheck", "pnpm run-script typecheck"] {
            assert_eq!(
                rewrite_command_no_prefixes(&format!("{command} --noEmit"), &[]),
                Some("rtk pnpm typecheck --noEmit".into()),
