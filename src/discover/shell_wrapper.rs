...
/// Shell options whose value is a separate argument. Without these the scan
/// below stops at the first non-`-` word, so `bash -o pipefail -c '…'` hides
/// its `-c` behind `pipefail` and reads as "not a command-string wrapper".
const OPTIONS_TAKING_AN_ARGUMENT: &[&str] = &["-o", "+o", "-O", "+O", "--rcfile", "--init-file"];
...
pub(crate) fn is_shell_wrapper_candidate(command: &str) -> bool {
    ...
    let mut options = rest.split_ascii_whitespace();
    while let Some(option) = options.next() {
        ...
        if OPTIONS_TAKING_AN_ARGUMENT.contains(&option) {
            options.next();
        }
    }
    false
...
mod tests {
    ...
    fn test_shell_wrapper_candidate_includes_unsupported_command_options() {
        for command in [
            ...
            // An option whose value is a separate argument must not hide the
            // `-c` behind it (#3828 review).
