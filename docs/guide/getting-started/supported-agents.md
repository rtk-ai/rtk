...
Creates `~/.config/opencode/plugins/rtk.ts`, using the plugin API that matches the
detected OpenCode major version:
- `tool.execute.before` (V1)
- `Plugin.define`/`ctx.tool.hook("execute.before")` (V2).
