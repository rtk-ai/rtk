...
- First-batch generated PowerShell source is capped at 8 KiB UTF-8 and the complete encoded host command line is capped at 30,000 UTF-16 code units. The sizing math is intentional: 8 KiB source becomes about 16 KiB of UTF-16LE bytes and about 21.4 KiB of base64 text before `powershell -NoProfile -EncodedCommand` argv overhead, leaving headroom below the Windows 32,767-unit process command-line limit. Exceeding either cap returns exit code `2` with guidance to write a `.ps1` file and use explicit `powershell -File`; never truncate script text.
