...
Owns: per-agent hook scripts and configuration files for 9 supported agents (Claude Code, Copilot, Cursor, Cline, Windsurf, Codex, OpenCode, Hermes, Auggie).
...
- **[`auggie/`](auggie/README.md)** — Rust binary hook, `PreToolUse` with `launch-process` matcher, `settings.json` patching
...
| Augment Code (Auggie) | Rust binary (`rtk hook auggie`) | Transparent rewrite | Yes (`updatedInput`) |
