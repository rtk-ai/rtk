#!/usr/bin/env bash
# rtk-hook-version: 2
# RTK hook — rewrites commands to use rtk for token savings.
# Supports both Claude Code (PreToolUse:Bash) and Cursor (preToolUse:Shell).
...
if [ -n "$CURSOR_MODE" ]; then
  # Cursor preToolUse: https://docs.cursor.com/context/hooks
  jq -n \
