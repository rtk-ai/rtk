...
#[cfg(unix)]
#[test]
fn a_taken_backup_slot_pushes_the_install_to_a_numbered_one() {
    let project = TempDir::new().unwrap();
    let extension_dir = project.path().join(".pi/extensions");
    std::fs::create_dir_all(&extension_dir).unwrap();
    let extension = extension_dir.join("rtk.ts");
    ...
    // Occupy `.bak` and `.bak.1` through `.bak.8`, each with distinct content so none of them
    // can be handed back by the identical-content shortcut. What this asserts is that the
    // install lands on the first free slot; that the very last one is reachable belongs with
    // the probe, where the ceiling is visible.
    ...
    assert!(
        install.status.success(),
        "an install must use the first free slot: {}",
        stderr(&install)
    );
