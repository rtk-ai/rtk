...
main.rs
  └─ CcEconomics { daily, weekly, monthly, all, format }
       └─ cc_economics::run()
            ...
            ├─ merge_monthly(cc, rtk)                  // HashMap merge
...
- Shows RTK data only (columns with `—` for missing ccusage data)
...
fn merge_monthly(cc: Option<Vec<CcusagePeriod>>, rtk: Vec<MonthStats>) -> Vec<PeriodEconomics> {
    ...
    // Insert ccusage → merge rtk → sort by key
    // ...
}
```
...
- PeriodEconomics helpers → `.set_ccusage()`, `.set_rtk_from_*()`, `.compute_dual_metrics()`
